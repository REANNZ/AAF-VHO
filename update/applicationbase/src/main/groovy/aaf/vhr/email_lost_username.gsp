...
<g:if test="${managedSubject}">
  ...
  If you have also lost your password, you may initiate a password reset using the <strong>Recover your lost password</strong> link on the Tuakiri Virtual Home login page.
  </g:if>
  <g:else>
  <h5>Your account does not have a username yet.</h5>

  This may be because you have not finalized your account invite.  If you cannot find the original email with the account invite, please contact tuakiri@reannz.co.nz to request having the account invite sent again.
  </g:else>
</g:if>
<g:else>
  <h5>Your email address is not associated with an account in Tuakiri Virtual Home.</h5>
